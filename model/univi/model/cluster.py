...
def cluster_dpc_knn(token_dict, cluster_num, k=5, token_mask=None):      # knn聚类
    ...
    with torch.no_grad():
        ...
        dist_matrix = torch.cdist(x.float(), x.float()) / (C ** 0.5)     # bs, n, n
        ...
        dist_nearest, index_nearest = torch.topk(dist_matrix, k=k, dim=-1, largest=False)    # bs, n, k， 最近的k个点
        density = (-(dist_nearest ** 2).mean(dim=-1)).exp()      # get the density of each token
        ...
        idx_batch = torch.arange(B, device=x.device)[:, None].expand(B, cluster_num)         # bs, num_cluster
        idx_tmp = torch.arange(cluster_num, device=x.device)[None, :].expand(B, cluster_num)          # bs, num_cluster
...
class CTM(nn.Module):
    ...
    def forward(self, token_dict, sample_ratio=None):
        ...
        token_weight = token_weight.unsqueeze(2)      # bs, n, 1
        token_dict['x'] = x

        if sample_ratio is not None:        # 采样后的最大token数量
