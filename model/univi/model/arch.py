...
class MetaModel:
    def __init__(self, config):
        ...
        if hasattr(config, "config"):
            ...
            if self.use_cluster:
                ...
                self.ctm4 = CTM(sample_ratio=8, embed_dim=self.config.mm_hidden_size, dim_out=self.config.mm_hidden_size, k=3)
                self.block4 = TCBlock(dim=self.config.mm_hidden_size, num_heads=8)
        else:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
       ...
       #  vision_tower = './checkpoints/sam_vit_h_4b8939.pth'
    ...
    def initialize_cluster_modules(self, model_args):
        ...
        if self.use_cluster and not hasattr(self, 'ctm0'):
            ...
            # no trainable parameters
            self.ctm4 = CTM(sample_ratio=model_args.spatial_cluster_rate3, embed_dim=self.config.mm_hidden_size, dim_out=self.config.mm_hidden_size, k=3)
            self.block4 = TCBlock(dim=self.config.mm_hidden_size, num_heads=8)
...
class ChatUniViMetaForCausalLM(ABC):
    ...
    def project(self, image_features, input_type="image"):
        if self.get_model().use_cluster:
            if input_type == "image":        # 当输入是图像时
                cluster_image_features = []
                condence_image_features = []
                token_dict = {'x': image_features,
                              'token_num': image_features.size(1),      # num_tokens
                ...
                condence_dict = self.get_model().block3(self.get_model().ctm3(token_dict))
                condence_image_features.append(condence_dict["x"])
                condence_image_features = torch.cat(condence_image_features, dim=1)

                image_features = torch.cat(cluster_image_features, dim=1)      # t, num_tokens, dim
        ...
        condence_image_features = self.get_model().mm_projector(condence_image_features)
        return image_features
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        concat_images = torch.cat(images, dim=0)         # num_conv2 * t2 + num_conv1 * t1, 3, h, w
        image_features = self.encode_images(concat_images)       # num_conv * t, n, dim 使用clip编码视觉特征
        ...
        cur_image_idx = 0
        for batch_idx, cur_input_ids in enumerate(input_ids):    # num_conv, num_tokens      for each conv
            if (cur_input_ids == IMAGE_TOKEN_INDEX).sum() == 0:        # 判断token中是否含有image
            ...
            if labels is not None:        # 确保token长度一致
            ...
            if len(image_token_indices) > 0:
                while True:
                    image_token_indices = torch.where(cur_input_ids == IMAGE_TOKEN_INDEX)[0]     # 一个conv中有几张图像，这里相应就会有几个索引
                    if image_token_indices.numel() == 0:     # 检查元素数量是否为零
                        break         # 若不包含图像，则跳出循环

                    i = [image_token_indices[0]]  # 取第一个图像标记的位置索引
                    image_token_start = i[0]  # 设定图像标记的起始位置
                    image_token_end = i[-1]   # 设定图像标记的结束位置（对于单一标记，起始和结束位置相同）
                    cur_image_features = []  # 初始化当前图像特征列表

                    for _ in i:    # 遍历 i 中的每个索引（实际上这里 i 中只有一个索引）
                        cur_image_features.append(image_features[cur_image_idx])  # 将对应的图像特征添加到列表中    256, 1024
                        cur_image_idx += 1  # 更新当前图像特征索引
                    ...
                    if len(i) > 2:
                        ...
                        cur_image_features = torch.stack(cur_image_features, dim=0)      # 1, 256, dim
                        cur_image_features = self.project(cur_image_features, input_type="image")      # 对image token进行聚类和投影
                        t, l, n = cur_image_features.size()
                        cur_image_features = cur_image_features.contiguous().view(t * l, n)

                    cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids[:image_token_start]))     # 对text token 进行嵌入得到embedding
                    cur_new_input_embeds.append(cur_image_features)         # 添加投影后的图像特征
                    cur_input_ids = cur_input_ids[image_token_end + 1:]     # 剩余的文本to
                    if labels is not None:
            ...
            if cur_input_ids.numel() > 0:    # 元素数量不为空
                cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids))      # token embedding
                if labels is not None:
