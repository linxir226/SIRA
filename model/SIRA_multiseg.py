...
class SiraMetaModel:
    def __init__(
        ...
        **kwargs,
        ...
        super(SiraMetaModel, self).__init__(config) # 意味着先调用父类初始化，因为上一个函数有两个父类，所以这里是调用ChatUniViLlamaModel的模型初始化函数
...
class SiraModel(SiraMetaModel, ChatUniViLlamaModel):
    def __init__(
        self,
        config,
        **kwargs,
    ):
        super(SiraModel, self).__init__(config, **kwargs) # 优先调用父类初始化函数，只调用第一个，第二个的会融入第一个的调用中，是父类的父类
...
# ├── self.model = SiraModel（你真正用来产出 hidden_states + 承载分割模块的“内核模型”）
# │ ├── SiraMetaModel（装配 LISA/SAM 相关模块的 mixin）
...
# ├── self.lm_head = nn.Linear(hidden_size→vocab_size)（类tokenid映射，在 SiraForCausalLM 里重新定义/覆盖的 lm_head）
# │
...
class SiraForCausalLM(ChatUniViLlamaForCausalLM): # HF-style causal LM 封装类，负责：forward，generate，loss 计算，tokenizer 对齐，lm_head
    def __init__(
        self,
        config,
        **kwargs,
        ...
        self.model = SiraModel(config, **kwargs) # 1.ChatUniVi模型主体初始化，（2.SAM2视觉模型初始化，3.文本→分割投影MLP初始化）后面两个在外部train.py初始化
