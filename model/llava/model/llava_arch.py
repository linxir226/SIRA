...
from dataset.utils import (DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN,
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            concat_images = torch.cat([image for image in images], dim=0)        # num_conv1 * t1 + num_conv2 * t2, 3, 224, 224
            image_features = self.encode_images(concat_images)           # num_conv1 * t1 + num_conv2 * t2, 256, 4096
