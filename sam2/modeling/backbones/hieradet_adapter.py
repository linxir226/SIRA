...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkv = self.qkv(x)
        qkv = qkv.reshape(B, H * W, 3, self.num_heads, qkv.shape[-1] // 3 // self.num_heads)
        # q, k, v with shape (B, H * W, nheads, C)
        q, k, v = torch.unbind(qkv, 2)

        # Q pooling (for downsample at stage changes)
        if self.q_pool:
            B, HW, nheads, C = q.shape
            q = do_pool(q.reshape(B, H, W, nheads * C), self.q_pool)
            H, W = q.shape[1:3]  # downsampled shape
            q = q.reshape(B, H * W, self.num_heads, C)
        ...
        if x.shape[0] != 0:
            ...
            attn = q @ k.permute(0, 2, 3, 1)  # B x N_heads x N_tokens x N_tokens
        ...
        # Transpose back
        x = x.transpose(1, 2)
        c_per_head = x.shape[-1]
        x = x.reshape(B, H, W, self.num_heads * c_per_head)
...
################ 方案一并非严格的adapter，因为加在了相邻的模块中间 ##################

class CrossModalAttention1(nn.Module):
    def __init__(self, visual_dim, seg_token_dim, key_dim, value_dim, out_dim, num_heads=1):
        super(CrossModalAttention1, self).__init__()
...
class AdapterBlock1(nn.Module):
    def __init__(self, dim, visual_in_dim, seg_token_dim, num_heads, dropout):
        super(AdapterBlock1, self).__init__()
        self.vis_project = nn.Sequential(nn.Conv1d(dim, dim, 1, 1),  # the init function sets bias to 0 if bias is True
                                         nn.GELU(),
                                         nn.Dropout(dropout))
        
        self.vis_lang_attn = CrossModalAttention1(visual_in_dim, seg_token_dim, key_dim=dim, value_dim=dim, out_dim=dim, num_heads=num_heads)
        
        self.out_project = nn.Sequential(nn.Conv1d(dim, dim, 1, 1),
                                        nn.GELU(),
                                        nn.Dropout(dropout))
    ...
    def forward(self, visual_feat: torch.Tensor, seg_token: torch.Tensor):
        ...
        # seg_token: bs, num_tokens, seg_token_dim
...
class CrossModalAttention2(nn.Module):
    def __init__(self, visual_dim, seg_token_dim, key_dim, value_dim, out_dim, num_query=5, num_heads=1):
        ...
        self.prompt_query = nn.Embedding(num_query, self.seg_token_dim)
        self.self_attn = nn.MultiheadAttention(self.seg_token_dim, self.num_heads)
        self.ffn = nn.Sequential(
            nn.Linear(seg_token_dim, seg_token_dim),
            nn.GELU(),
            nn.Linear(seg_token_dim, seg_token_dim)
        )
        self.norm1 = nn.LayerNorm(seg_token_dim)
        self.norm2 = nn.LayerNorm(seg_token_dim)
    ...
    def forward(self, visual_feat: torch.Tensor, seg_token: torch.Tensor):
        ...
        # 先利用自注意力机制生成多个query
        assert visual_feat.shape[0] == seg_token.shape[0]
        prompt_query = self.prompt_query.weight[None, :, :].repeat(visual_feat.shape[0], 1, 1)    # bs, num_query, seg_token_dim
        prompt_query_ = torch.cat((seg_token, prompt_query), dim=1).transpose(0, 1)     # 1 + num_query, bs, seg_token_dim
        prompt_query_ = self.self_attn(prompt_query_, prompt_query_, prompt_query_)[0][1:, :, :]     # num_query, bs, seg_token_dim
        
        prompt_query = self.norm1(prompt_query + prompt_query_.transpose(0, 1))     # bs, num_query, seg_token_dim
        out_query = self.norm2(prompt_query + self.ffn(prompt_query))     # bs, num_query, seg_token_dim
        ...
        key = self.f_key(out_query).permute(0, 2, 1)  # (B, key_channels, 1)
        value = self.f_value(out_query).permute(0, 2, 1)  # (B, self.value_channels, 1)
        n_l = value.size(-1)
        # (b, num_heads, H*W, self.key_channels//self.num_heads)
        query = query.reshape(B, HW, self.num_heads, self.key_dim // self.num_heads).permute(0, 2, 1, 3)
        # (b, num_heads, self.key_channels//self.num_heads, 1)
...
class AdapterBlock2(nn.Module):
    def __init__(self, dim, visual_in_dim, seg_token_dim, num_heads, dropout):
        ...
        self.vis_lang_attn = CrossModalAttention2(visual_in_dim, seg_token_dim, key_dim=dim, value_dim=dim, out_dim=dim, num_heads=num_heads)
