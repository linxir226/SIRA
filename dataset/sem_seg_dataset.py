...
def init_mapillary(base_image_dir):
    ...
    mapillary_labels = sorted(        # 读出所有的标签并排序
        glob.glob(
...
def init_ade20k(base_image_dir):
    with open("dataset/ade20k_classes.json", "r") as f:
...
def init_cocostuff(base_image_dir):
    # 读取coco_stuff中的类别名称
    cocostuff_classes = []
    with open("dataset/cocostuff_classes.txt") as f:
...
def init_paco_lvis(base_image_dir):
    ...
    all_classes = coco_api_paco_lvis.loadCats(coco_api_paco_lvis.getCatIds())        # 加载出类别名称
    class_map_paco_lvis = {}
    ...
    img_ids = coco_api_paco_lvis.getImgIds()       # read each class and super category
...
class SemSegDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for ds in self.sem_seg_datas:        # cocostuff, pascal part, ade20k, paco_lvis, mapillary
    ...
    def __getitem__(self, idx):
        ...
        for sampled_cls in sampled_classes:
            ...
            # t_question = question_template.format(sent=text.lower())
