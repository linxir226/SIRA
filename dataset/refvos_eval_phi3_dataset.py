...
def collate_fn_(
    batch, tokenizer=None, conv_type="phi3_instruct", use_mm_start_end=True, clip_model=None, local_rank=-1
):
    image_path_list = []
    images_list = []
    images_clip_list = []
...
class RVOSEvalDataset(torch.utils.data.Dataset):
    ...
    def load_target_frame(self, ):
        metas, _, _, _ = load_refytvos_json(self.image_root, self.json_file, self.rvos_dataset_name)
        ...
        for idx_vd, vid_dict in tqdm(enumerate(metas), desc=f'Loading {self.rvos_dataset_name} ...'):
            ...
            d2_dataset_dicts.append(record)       # 长度为len(exp)，用来存储各exp的内容，ID，对应视频，以及视频帧
            
            # 取关键帧
            # target_frame_ids = int(vid_dict['tp'] * vid_dict['length'])
            # if target_frame_ids == vid_dict['length']:
            #     target_frame_ids = target_frame_ids - 1
                  
            # clip_frame_list = [Image.open(frame_path) for frame_path in record['file_names']]
    ...
    def __getitem__(self, idx):
        ...
        #     idx_start = index_frame % step_size
        #     idx_select = list(range(idx_start, len(data['video_frame_path_list']), step_size))      # 以step size为间隔，等距离抽帧
        # else:
        #     idx_select = [index_frame, ]
        # assert index_frame in idx_select
        ...
        condition_index = 0 # data['video_frame_path_list'].index(data['seg_frame_path'])    # 条件帧在所有帧的索引
...
if __name__ == "__main__":
    image_root = os.environ.get("RVOS_IMAGE_ROOT", "./data/rvos_root")
    json_file = os.environ.get(
        "RVOS_JSON_FILE",
        "./data/rvos_root/ReVOS/meta_expressions_valid__llamavid.json",
    )
    dataset_name = 'revos_valid'
    output_dir = os.environ.get("RVOS_OUTPUT_DIR", "./outputs/rvos_eval")
    metas, _, _, _ = load_mevis_json(image_root, json_file, dataset_name)
    
    d2_dataset_dicts = []
    lisa_dataset_dicts = []
    
    tmp_valid_vid = 0
    for idx_vd, vid_dict in tqdm(enumerate(metas), desc=f'Loading {dataset_name} ...'):
        record = {}
        if (dataset_name == "refytvos_valid") and (vid_dict['video'] not in REFYTVOS_VAL_VIDEOS):
            continue
        record["file_names"] = [
        ...
        target_image = vid_dict['frames'][target_frame_ids]   
        lisa_dataset_dicts.append(
            dict(
                idx_d2 = tmp_valid_vid, 
                frame_path = record['file_names'][target_frame_ids], 
                output_path = osp.join(output_dir, "Annotations", video_name, exp_id, target_image + '.png')
            )
        )
        
        for file_path in record["file_names"]:
            file_name = osp.basename(file_path).rsplit('.', 1)[0]
            output_path = osp.join(output_dir, "Annotations", video_name, exp_id, file_name + '.png')
            lisa_dataset_dicts.append(          # 长度为每一exp对应的视频帧数之和
                dict(
                    idx_d2 = tmp_valid_vid,
