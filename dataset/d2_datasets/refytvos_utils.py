    ...
    def prepare_metas():
        if ('train' in dataset_name) or is_train:
            ...
            with open(os.path.join(str(img_folder), 'meta.json'), 'r') as f:
                subset_metas_by_video = json.load(f)['videos']      # 抽取出videos对应的列表，里面保存着各个视频对应的物体种类和索引
            
            # read expression data
            with open(str(ann_file), 'r') as f:
                subset_expressions_by_video = json.load(f)['videos']    # 里面保存各个视频中物体对应的expression以及种类
            ...
            for vid in videos:       # 对于每个视频
                vid_meta   = subset_metas_by_video[vid]      # 该视频中标注的物体类别信息，"bear": {"objects": {"1": {"category": "bear"}}}
                vid_data   = subset_expressions_by_video[vid]     # "bear": {"expressions": {"0": {"exp": , "obj_id": }, "1": , "2": }, "frames": {}
                vid_frames = sorted(vid_data['frames'])
                vid_len    = len(vid_frames)

                exp_id_list = sorted(list(vid_data['expressions'].keys()))     # 对视频中的所有expression排序
                for exp_id in exp_id_list:
                    exp_dict            = vid_data['expressions'][exp_id]
                    meta                = {}
                    meta['video']       = vid             # 视频名称
                    meta['exp']         = exp_dict['exp']             # expressions
                    meta['obj_id']      = [0, ]           # Ref-Youtube-VOS only has one object per expression
                    meta['anno_id']     = [str(anno_count), ]
                    anno_count         += 1
                    meta['frames']      = vid_frames          # 视频帧数
                    meta['exp_id']      = exp_id             # expression索引
                    obj_id              = exp_dict['obj_id']          # expression对应的物体类别
                    meta['obj_id_ori']  = int(obj_id)
                    meta['category']    = vid_meta['objects'][obj_id]['category']
                    meta['length']      = vid_len
                    metas.append(meta)
                    vid2metaid[vid].append(len(metas) - 1)        # 每个视频对应的exp的索引
        else:
            ...
            if 'refytvos' in dataset_name:
                ...
                meta_file = ann_file # os.path.join(root, "meta_expressions", split, "meta_expressions.json")
                data = json.load(open(meta_file, 'r'))["videos"]
